...
class eMPL_packet_reader:
    ...
    def __init__(self, clkbAddr, clkbPort, quat_delegate=None, debug_delegate=None, data_delegate=None ):
        self.s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.s.connect((clkbAddr, clkbPort))
...
# TODO: Will this break anything?
    ...
    def read(self):
        ...
        rs = self.s.recv(BUFSIZE).decode('utf-8')
...
class data_packet (object):
    ...
    def display(self):
        ...
        if self.type == 0:
            ...
            print('rotation matrix: \n%7.3f %7.3f %7.3f\n%7.3f %7.3f %7.3f\n%7.3f %7.3f %7.3f' % (self.data[0], self.data[1], self.data[2], self.data[3], self.data[4], self.data[5], self.data[6], self.data[7], self.data[8]))
...
class quat_packet (object):
    ...
    def __init__(self, l):
        ...
        quatArray = dataRegex.findall(quatPacket)[0]

        self.l = l

        self.q0 = float(quatArray[0])#four_bytes(l[3],l[4],l[5],l[6]) * 1.0 / (1<<30)

        self.q1 = float(quatArray[1])#four_bytes(l[7],l[8],l[9],l[10]) * 1.0 / (1<<30)

        self.q2 = float(quatArray[2])#four_bytes(l[11],l[12],l[13],l[14]) * 1.0 / (1<<30)

        self.q3 = float(quatArray[3])#four_bytes(l[15],l[16],l[17],l[18]) * 1.0 / (1<<30)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        print("usage: " + sys.argv[0] + " addr port")
    ...
    reader = eMPL_packet_reader(addr,port, 
